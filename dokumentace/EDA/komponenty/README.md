...
- **Webový server**
    - Jde o komponentu či komponenty (neuvažujeme load balancing), která slouží pro hostování webového API naší aplikace.
    - Hostuje mediátora, čímž funguje jako „prostředník“ mezi klientskou aplikací a aplikačním serverem.
- **Mediátor**
    - V rámci architektonického rozhodnutí o celkové architektuře byla vybrána architektura EDA s topologií mediátor. Tato komponenta zajišťuje výměnu zpráv mezi klientskou aplikací a jednotlivými službami na aplikačním serveru a řídí jednotlivé eventy. 
- **Aplikační server**
    - Jde o komponentu či komponenty (neuvažujeme load balancing), které slouží pro hostování samotných služeb naší aplikace.
- **Slevová služba**
